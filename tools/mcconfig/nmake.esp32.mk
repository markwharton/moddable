...
!IF "$(XSBUG_LOG)"=="1"
!IF "$(USE_USB)"=="0"
START_SERIAL2XSBUG = echo Launching app... && set "XSBUG_LOG_PORT=$(XSBUG_LOG_PORT)" && set "XSBUG_PORT=$(XSBUG_PORT)" && set "XSBUG_HOST=$(XSBUG_HOST)" && set "XSBUG_PROJECT=$(MAIN_DIR)" && $(MODDABLE)\build\bin\win\release\xsdb start /B $(BUILD_DIR)\bin\win\release\serial2xsbug $(PORT_TO_USE) $(DEBUGGER_SPEED) 8N1
!ELSE
START_SERIAL2XSBUG = echo Launching app... && set "XSBUG_LOG_PORT=$(XSBUG_LOG_PORT)" && set "XSBUG_PORT=$(XSBUG_PORT)" && set "XSBUG_HOST=$(XSBUG_HOST)" && set "XSBUG_PROJECT=$(MAIN_DIR)" && $(MODDABLE)\build\bin\win\release\xsdb start /B $(BUILD_DIR)\bin\win\release\serial2xsbug $(USB_VENDOR_ID):$(USB_PRODUCT_ID) $(DEBUGGER_SPEED) 8N1
!ENDIF
START_XSBUG =
!ENDIF
