		...
		if (this.log.endsWith("\n")) {
			if (this.log.startsWith("|:") && this.log.endsWith(":|\n")) {
				...
				if (payload.startsWith("@filename ")) {
					...
					if (!this.binaryOutput) {
						this.binaryPath = Host.getenv("MODDABLE") + "/build/tmp/" + (this.binaryName ? this.binaryName : "log.bin");
						File.delete(this.binaryPath);
						this.binaryOutput = new File(this.binaryPath, true);
					}
					this.binaryOutput.write(Uint8Array.fromBase64(payload).buffer);
				}
			}
			else
